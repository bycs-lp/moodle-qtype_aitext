...
final class question_test extends \advanced_testcase {
     ...
     *
     * process_feedback() now stores the feedback raw (as returned by the model): the JSON is
     * extracted and the disclaimer appended, but no Markdown/filter formatting is applied. That
     * happens only at display time in {@see \qtype_aitext\comment_formatter::to_html()}.
     *
    ...
    public function test_process_feedback(
        ...
        $this->resetAfterTest();
        // Ensure the mathjaxloader filter is globally on.
        ...
        if (empty($json)) {
            ...
            // Normal case: feedback is stored raw and the disclaimer is appended, no formatting.
            $expectedfeedbackformatted = comment_formatter::to_html(
                $expectedfeedback . ' (example disclaimer)',
                FORMAT_MARKDOWN,
                \context_system::instance(),
            );
            $this->assertEquals(
                $expectedfeedbackformatted,
                comment_formatter::to_html($processedfeedback->feedback, FORMAT_MARKDOWN, \context_system::instance())
            );
    ...
    public static function process_feedback_provider(): array {
        return [
            ...
            // through format_text when displaying, which again will strip one backslash.
